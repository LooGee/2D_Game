...
public class EnemyMain : MonoBehaviour
{
    ...
    {
        //Dog pile Set
        Stage_DogPile[] dogPileList = GameObject.Find("DogPile").GetComponentsInChildren<Stage_DogPile>();
        foreach(Stage_DogPile findDogPile in dogPileList)
        {
    ...
    public bool BeginEnemyCommonWork()
    {
        ...
        if(dogPile != null)
        {
            if(GetDistanePlayer() > dogPileReturnLength)
    ...
    public float GetDistanePlayer()  //플레이어와의 현재 거리
    {
    ...
    public bool IsChangeDistanePlayer(float l) //거리가 변했는지 확인하기
    {
        return (Mathf.Abs(distanceToPlayer - distanceToPlayerPrev) > l);
    }

    public float GetDistanePlayerX()  //플레이어와의 현재 x거리
    {
    ...
    public float GetDistanePlayerY()  //플레이어와의 현재 y거리
    {
        Vector3 posA = transform.position;
        Vector3 posB = playerCtrl.transform.position;
        posA.x = 0; posA.z = 0;
